...
export default function Week({ viewedWeek, addEventHandler, timesheetStatus, events }) {
    ...
    for (let i = 0; i < weekDays.length; i++) {
        // Check if events is defined before trying to filter it
        const filteredEvents = events ? events.filter(event => event.date === weekDates[i]) : [];
        week.push(
            <Day
                key={i + 1}
                day={weekDays[i]}
                ...
                events={filteredEvents} // Pass filtered events to Day component
            />
        );
    }
